...
@media (prefers-reduced-motion:reduce){:where(.wb-spinner,.wb-dialog,.wb-dialog-overlay,.wb-menu,.wb-tooltip){animation:none}}
`;
  ...
  const MuiButton = React.forwardRef(function WbMuiButton(p, ref) {
    const { variant = 'text', gradient, startIcon, endIcon, disabled, loading, onClick, children, className, style, testId, noHover, type = 'button', ...rest } = p;
    return (
      <button
        ref={ref}
        type={type}
        className={cx('wb-mbtn', 'wb-mbtn-' + variant, gradient && 'wb-mbtn-gradient', noHover && 'wb-mbtn-nohover', loading && 'wb-mbtn-loading', className)}
        style={style}
        ...
        {...rest}
      >
        {startIcon ? <span className="wb-mbtn-start">{startIcon}</span> : null}
        <span>{children}</span>
        {endIcon ? <span className="wb-mbtn-start" style={{ margin: '0 -4px 0 4px' }}>{endIcon}</span> : null}
        {loading ? <span className="wb-spin" /> : null}
      </button>
    );
  });

  const MuiIconButton = React.forwardRef(function WbMuiIconButton(p, ref) {
    const { size = 'small', label, onClick, disabled, className, style, testId, children, active, ...rest } = p;
    return (
      <button
        ref={ref}
        type="button"
        ...
        onClick={onClick}
        data-testid={testId}
        className={cx('wb-micon', 'wb-micon-' + (size === 'small' ? 'sm' : 'md'), className)}
        style={style}
        {...rest}
      >
        {children}
  ...
  const MuiTextField = ({ label, placeholder, value, onChange, size = 'small', fullWidth, startAdornment, endAdornment, disabled, error, helperText, type = 'text', testId, style, className, multiline, rows, inputProps, onKeyDown, autoFocus }) => {
    const [focused, setFocused] = useState(false);
    const filled = value !== undefined && value !== null && String(value) !== '';
    const shrunk = focused || filled || !!startAdornment || !!placeholder;
    const InputTag = multiline ? 'textarea' : 'input';
    return (
      <div className={cx('wb-mf', fullWidth && 'wb-mf-full', shrunk && 'wb-mf-shrunk', error && 'wb-mf-error', disabled && 'wb-mf-disabled', className)} style={style} data-testid={testId ? testId + '-root' : undefined}>
        <div className="wb-mf-ctrl">
          {startAdornment ? <span className="wb-mf-start">{startAdornment}</span> : null}
          <InputTag
            className={cx('wb-mf-input', startAdornment && 'wb-mf-has-start')}
            ...
            style={multiline ? { height: 'auto', minHeight: 40, resize: 'vertical', lineHeight: 1.5, paddingTop: 10, paddingBottom: 10 } : undefined}
            {...(inputProps || {})}
          />
          {endAdornment ? <span className="wb-mf-end">{endAdornment}</span> : null}
          <fieldset className="wb-mf-outline" aria-hidden="true"><legend><span>{label || ' '}</span></legend></fieldset>
        </div>
        {label ? <label className="wb-mf-label">{label}</label> : null}
        {helperText ? <div className="wb-mf-helper">{helperText}</div> : null}
      </div>
    );
  };

  const MuiSelect = ({ label, value, onChange, options, placeholder, renderValue, fullWidth, disabled, size = 'small', testId, style, className, displayEmpty, menuWidth }) => {
    const [open, setOpen] = useState(false);
    const anchorRef = useRef(null);
    const [pos, setPos] = useState(null);
    const list = Array.isArray(options) ? options : [];
    const selected = list.find((o) => String(o.value) === String(value));
    const shrunk = displayEmpty || (value !== undefined && value !== null && String(value) !== '') || open;
    ...
    useEffect(() => {
      ...
      const place = () => {
        ...
        const maxH = 320;
        const top = below > Math.min(maxH, 200) + 8 ? r.bottom + 4 : Math.max(8, r.top - Math.min(maxH, 320) - 4);
        setPos({ left: r.left, top, width: menuWidth || r.width });
      };
      place();
      const close = (e) => { if (anchorRef.current && anchorRef.current.contains(e.target)) return; if (e.target.closest && e.target.closest('.wb-menu-pop')) return; setOpen(false); };
      const onKey = (e) => { if (e.key === 'Escape') setOpen(false); };
      window.addEventListener('mousedown', close);
      window.addEventListener('keydown', onKey);
      window.addEventListener('resize', place);
      window.addEventListener('scroll', place, true);
      return () => { window.removeEventListener('mousedown', close); window.removeEventListener('keydown', onKey); window.removeEventListener('resize', place); window.removeEventListener('scroll', place, true); };
    }, [open, menuWidth]);

    const shown = selected
      ? (typeof renderValue === 'function' ? renderValue(selected) : (
        <>
          {selected.icon ? <span className="wb-menu-item-icon">{selected.icon}</span> : null}
          <span style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{selected.label}</span>
        </>
      ))
      : (typeof renderValue === 'function' ? renderValue(null) : placeholder || '');

    const menu = open && pos ? (
      <div className="wb-menu-pop" role="listbox" style={{ left: pos.left, top: pos.top, minWidth: pos.width }} data-testid={testId ? testId + '-menu' : undefined}>
        {list.map((o) => (
          <div
            key={String(o.value)}
            role="option"
            ...
            onClick={() => { if (o.disabled) return; setOpen(false); if (typeof onChange === 'function') onChange(o.value); }}
          >
            {o.flagSrc ? <img className="wb-menu-item-flag" src={o.flagSrc} alt="" /> : null}
            {o.icon ? <span className="wb-menu-item-icon">{o.icon}</span> : null}
            <span style={{ flex: '1 1 auto', minWidth: 0 }}>{o.label}{o.hint ? <span className="wb-menu-item-hint">{' ' + o.hint}</span> : null}</span>
            {o.cost !== undefined && o.cost !== null ? <span className="wb-menu-item-cost">{typeof o.cost === 'number' ? '$' + o.cost.toFixed(2) : o.cost}</span> : null}
          </div>
        ))}
      </div>
    ) : null;
    ...
    return (
      <div className={cx('wb-mf', 'wb-mf-select', fullWidth && 'wb-mf-full', shrunk && 'wb-mf-shrunk', open && 'wb-mf-select-open', disabled && 'wb-mf-disabled', className)} style={style} ref={anchorRef}>
        <div className="wb-mf-ctrl">
          <button
            type="button"
            className={cx('wb-mf-input', !selected && 'wb-mf-placeholder')}
            disabled={disabled}
          ...
          <span className="wb-mf-caret"><LucideIcon name="chevron-down" size={22} /></span>
          <fieldset className="wb-mf-outline" aria-hidden="true"><legend><span>{label || ' '}</span></legend></fieldset>
        </div>
        {label ? <label className="wb-mf-label">{label}</label> : null}
        {menu}
  ...
  return {
    ...
    MuiButton, MuiIconButton, MuiTextField, MuiSelect,
  };
};
