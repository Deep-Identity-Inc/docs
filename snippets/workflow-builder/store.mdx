export const makeStore = (deps) => {
  ...
  const TEMPLATES = (WB && WB.WORKFLOW_TEMPLATES) || [];
  ...
  const reduceApplyTemplate = (state, a) => {
    if (a.templateId === 'scratch') {
      return {
        ...state,
        workflow: { name: '', steps: [], canvasData: { connections: [], nodePositions: { [START]: { x: 400, y: 40 } }, viewport: blankViewport() } },
        selectedStepId: null,
        selectedConnectionId: null,
      };
    }
    const build = U('buildWorkflowFromTemplate', null);
    const tpl = TEMPLATES.find((t) => t.id === a.templateId);
    if (!tpl || typeof build !== 'function') return state;
    const built = build(a.templateId);
    const workflow = normalizeWorkflow(built);
    return { ...state, workflow, selectedStepId: null, selectedConnectionId: null };
  };
  ...
  const reducer = (state, action) => {
    ...
    switch (a.type) {
      case 'LOAD': {
        ...
        if (a.keepSelection) {
          const keys = new Set(workflow.steps.map(stepKey));
          const selectedStepId = keys.has(state.selectedStepId) ? state.selectedStepId : null;
          const selectedConnectionId = workflow.canvasData.connections.some((c) => c.id === state.selectedConnectionId) ? state.selectedConnectionId : null;
          return { ...state, workflow, viewMode, hydrated: true, selectedStepId, selectedConnectionId };
        }
      ...
      case 'PATCH_STEP': {
        const key = a.stepKey;
        const nextStep = a.step;
        if (!key || !isObj(nextStep)) return state;
        const steps = state.workflow.steps;
        if (!steps.some((s) => stepKey(s) === key)) return state;
        return withWorkflow(state, { steps: steps.map((s) => (stepKey(s) === key ? { ...s, ...nextStep, instanceId: s.instanceId } : s)) });
      }
  ...
  const ACTION_DEFS = {
    load: ['LOAD', ['workflow', 'viewMode', 'keepSelection']],
    reset: ['RESET', []],
    ...
    patchStep: ['PATCH_STEP', ['stepKey', 'step']],
  };
