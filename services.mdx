...
<FeatureGrid cols={2}>
  ...
  <FeatureCard icon="sim-card" title="Phone Trust" description="Asks the mobile carrier for recent SIM-swap and call-forwarding signals on a number — catching account takeovers before a code is ever sent. Passive, nothing for the applicant to do." />

  <FeatureCard icon="cake" title="Carrier Age Gate" description="The mobile carrier confirms the account holder is 18 or over without revealing their age — a yes/no answer straight from the network." />

  <FeatureCard icon="user-check" title="Phone Ownership" description="Scores how well an applicant's name, address and date of birth match the subscriber on record for their phone number, directly from the carrier." />
</FeatureGrid>
