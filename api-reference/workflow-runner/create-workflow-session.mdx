...
<Info>
  The Runner supports every step accepted by [Create Workflow](/api-reference/workflows/create-workflow). Read each returned step's `requirements` instead of hard-coding a two-step flow.
</Info>
...
The Runner handles the 29 workflow steps in four ways:

| Behavior | Step IDs | Client behavior |
|---|---|---|
| `client-submit` | `ID_VERIFICATION`, `AGE_ESTIMATION`, `CREDIT_CHECK`, `KYB`, `TITLE_SEARCH`, `PEP_SANCTIONS`, `IP_JURISDICTION`, `VPN_DETECTION`, `INJECTION_DETECTION`, `ANTI_CHEAT`, `CUSTOM_PROMPT`, `DOCUMENT_UPLOAD`, `CUSTOM_FORM` | Collect the fields or uploads described by `requirements`, then submit the current step |
...
| Status | Description |
|---|---|
| `400` | Invalid request body, an invalid workflow contract, or the workflow is not `active` |
