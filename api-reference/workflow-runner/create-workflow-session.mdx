...
<Info>
  The Runner supports 21 of the 22 steps accepted by [Create Workflow](/api-reference/workflows/create-workflow). `E_SIGNATURE` is hosted-only and cannot be executed through the headless Runner. Read each returned step's `requirements` instead of hard-coding a two-step flow.
</Info>
...
The Runner handles its 21 supported workflow steps in four ways:

| Behavior | Step IDs | Client behavior |
|---|---|---|
| `client-submit` | `ID_VERIFICATION`, `AGE_ESTIMATION`, `TITLE_SEARCH`, `PEP_SANCTIONS`, `CUSTOM_PROMPT`, `DOCUMENT_UPLOAD` | Collect the fields or uploads described by `requirements`, then submit the current step |
...
| `multi-phase` | `FACE_LIVENESS`, `DEEPFAKE_DETECTION`, `DEEP_AGE`, `ADDRESS_VERIFICATION`, `BACKGROUND_CHECK`, `ADVERSE_MEDIA`, `BANK_STATEMENT_UPLOAD`, `PHONE_VERIFICATION` | Follow the `actions` listed in `requirements` and read the updated requirements after each phase |
