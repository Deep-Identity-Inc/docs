...
The detailed examples below cover upload, presentation, server-auto, and multi-phase execution. For larger schemas such as `DEEPFAKE_DETECTION`, use the generated request schema on this page together with the current step's `requirements`.
...
| Step-specific result | object | executor-dependent | A snake_case key such as `phone_trust_check` or `phone_verification`. Treat new fields as additive |
