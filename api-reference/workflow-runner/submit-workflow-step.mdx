...
For config-driven files, pass the requirement's exact slot to [Create Session Uploads](/api-reference/workflow-runner/create-session-uploads), upload the file, then use the returned `file_key` in the step body.
...
When `accepted` is `true`, `legal_name` is required. If the workflow configuration requires a signature, request an upload URL with `slot: "signature_upload"`, upload it, and include its session-owned key as `signature_upload`. A declined consent completes the attempt with `failure_reason: "CONSENT_DECLINED"`.
