...
<Note>
  A workflow contains 1–10 unique steps. Repeated step IDs and advanced branching workflows are not supported. The API accepts only the production-ready steps listed below. Passport NFC Scanner and Crypto Wallet Screening are not part of the OpenAPI contract yet.
</Note>
...
The API also validates cross-step requirements. For example, `AI_BANK_STATEMENT_ANALYSIS` requires an earlier `BANK_STATEMENT_UPLOAD` step, and `BACKGROUND_CHECK` and `TITLE_SEARCH` require an earlier `ID_VERIFICATION` step. The [Workflow Builder](/workflow-builder) adds coupled prerequisites automatically.
...
```json 201
{
  "workflow": {
    ...
    "steps": [
      ...
      {
        "id": "AGE_ESTIMATION",
        "config": { "minimum_age": 18, "maximum_age": 100 }
      },
      { "id": "PEP_SANCTIONS", "config": {} },
      { "id": "ADVERSE_MEDIA", "config": {} }
    ]
  }
}
```
