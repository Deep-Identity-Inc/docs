...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `allowed_jurisdictions` | string[] | `[]` | ISO 3166 country codes that are always allowed (allowlist). When set, countries outside the list are treated as out-of-jurisdiction |
| `blocked_jurisdictions` | string[] | `[]` | ISO 3166 country codes to block |
| `out_of_jurisdiction_action` | string | `"block"` | Action when the resolved country is blocked (or not in the allowlist) |
| `datacenter_ip_action` | string | `"flag"` | Action when the IP resolves to a datacenter / hosting network |
...
| `confidence_threshold` | integer | `70` | Minimum confidence (1–100) to treat a hit as positive |
| `escalation_type` | string | `"none"` | Escalation type to run when the action is `step-up` |
...
| `confidence_threshold` | integer | `70` | Minimum confidence (1–100) to treat signals as a detection |
| `escalation_type` | string | `"none"` | Escalation type to run when the action is `step-up` |
